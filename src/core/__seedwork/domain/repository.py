...
class SearchParams(Generic[Filter]):
    ...
    def _convert_to_int(
            self, value: Any, default=0) -> int:  # pylint: disable=no-self-use
        try:
            return int(value)
...
class SearchResult(
        Generic[ET, Filter]):  # pylint: disable=too-many-instance-attributes
    items: List[ET]
    total: int
    current_page: int
    per_page: int
...
class InMemorySearchableRepository(
    ...
    def search(
            self,
            input_params: SearchParams[Filter]) -> SearchResult[ET, Filter]:
