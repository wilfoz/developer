...
class TestCategoryVAlidatorUnit(unittest.TestCase):
    ...
    def test_invalidation_cases_for_name_field(self):
        ...
        for i in invalid_data:
            ...
            self.assertListEqual(self.validator.errors['name'], [i['expected']],
    ...
    def test_invalidation_cases_for_create_at_field(self):
        is_valid = self.validator.validate({'create_at': None})
        self.assertFalse(is_valid)
        self.assertListEqual(self.validator.errors['create_at'], [
                             'This field may not be null.'])

        is_valid = self.validator.validate({'create_at': 5})
        self.assertFalse(is_valid)
        self.assertListEqual(self.validator.errors['create_at'], [
